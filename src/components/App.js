...
class App extends Component {
  ...
  async borrow(amount) {
    if (this.state.rombank !== 'undefined') {
      try {
        await this.state.rombank.methods.borrow().send({ value: amount.toString(), from: this.state.account })
      } catch (e) {
  ...
  async payOff(e) {
    ...
    if (this.state.rombank !== 'undefined') {
      try {
        const collateralEther = await this.state.rombank.methods.collateralEther(this.state.account).call({ from: this.state.account })
        const tokenBorrowed = collateralEther / 2
        await this.state.token.methods.approve(this.state.romBankAddress, tokenBorrowed.toString()).send({ from: this.state.account })
        await this.state.rombank.methods.payOff().send({ from: this.state.account })
      } catch (e) {
